...
const loadProxies = () => {
    ...
    for (const line of lines) {
        ...
        if (!p) {
            console.log(
                `[SYSTEM] WARNING: Invalid proxy format skipped: "${line}" - expected format: http://ip:port or user:pass@ip:port`
            );
            continue;
        }
...
const quarantineProxy = (idx, minutes = 15, reason = '') => {
    try {
        ...
        log(
            'SYSTEM',
            'wplacer',
            `🧯 Quarantining proxy #${idx} (${label}) for ${Math.floor(ms / 60000)}m${reason ? ` — ${reason}` : ''}`
        );
    } catch (_) {}
};
...
class WPlacer {
    constructor(
        ...
        this._burstSeeds = Array.isArray(initialBurstSeeds)
            ? initialBurstSeeds.map((s) => ({ gx: s.gx, gy: s.gy }))
            : null;
    ...
    _isCancelled() {
        try {
            return typeof this.shouldStop === 'function' ? !!this.shouldStop() : false;
        } catch (_) {
            return false;
        }
    }
    ...
    async login(cookies) {
        ...
        if (proxySel) {
            ...
            try {
                this._lastProxyIdx = proxySel.idx;
            } catch (_) {}
    ...
    async loadUserInfo() {
        ...
        if (status >= 300 && status < 400) {
            ...
            throw new Error(
                `❌ Unexpected redirect (${status})${loc ? ` to ${loc}` : ''}. Likely cookies invalid or blocked by proxy.`
            );
        }
        if (status === 401 || status === 403) {
            if (
                /cloudflare|attention required|access denied|just a moment|cf-ray|challenge-form|cf-chl/i.test(bodyText)
            ) {
                // auto-quarantine proxy for a short time to reduce repeated blocks
                try {
                    if (typeof this._lastProxyIdx === 'number')
                        quarantineProxy(this._lastProxyIdx, 20, `cloudflare_block_${status}`);
                } catch (_) {}
        ...
        if (contentType.includes('application/json')) {
            ...
            throw new Error(
                `❌ Unexpected JSON from /me (status ${status}): ${JSON.stringify(userInfo).slice(0, 200)}...`
            );
        }
        ...
        if (/cloudflare|attention required|access denied|just a moment|cf-ray|challenge-form|cf-chl/i.test(bodyText)) {
            try {
                if (typeof this._lastProxyIdx === 'number')
                    quarantineProxy(this._lastProxyIdx, 20, 'cloudflare_block_html');
            } catch (_) {}
            throw new Error(`❌ Cloudflare blocked the request.`);
        }
        if (/<!doctype html>/i.test(bodyText) || /<html/i.test(bodyText)) {
            throw new Error(
                `❌ Failed to parse server response (HTML, status ${status}). Likely a login page → cookies invalid or expired. Snippet: "${short}..."`
            );
        }
        ...
        const loadOneTile = async (targetTx, targetTy, allowActivate = true) => {
            try {
                ...
                if (resp.status === 404) {
                    ...
                    try {
                        log(
                            this.userInfo?.id || 'SYSTEM',
                            this.userInfo?.name || 'wplacer',
                            `[${this.templateName}] ⚠️ Tile ${targetTx}, ${targetTy} may be inactive (404). Trying to activate and reload...`
                        );
                    } catch (_) {}
                    return null;
                }
                    ...
                    if (res && res.success && res.painted > 0) {
                        try {
                            log(
                                this.userInfo.id,
                                this.userInfo.name,
                                `[${this.templateName}] ℹ️ Tile ${targetTx},${targetTy} might be inactive. Tried to activate by painting 1 pixel.`
                            );
                        } catch (_) {}
                        return true;
                    }
    ...
    async _executePaint(tx, ty, body) {
        ...
        if (response.data.painted && response.data.painted === body.colors.length) {
            ...
            try {
                ...
                const entry = Object.entries(templates).find(
                    ([tid, t]) =>
                        t &&
                        t.name === this.templateName &&
                        Array.isArray(t.coords) &&
                        JSON.stringify(t.coords) === JSON.stringify(this.coords)
                );
            ...
            log(
                this.userInfo.id,
                this.userInfo.name,
                `[${this.templateName}] ❌ Server error (500) - waiting before retry...`
            );
            await sleep(40000);
    ...
    _getMismatchedPixels() {
        ...
        for (let y = 0; y < this.template.height; y++) {
            for (let x = 0; x < this.template.width; x++) {
                ...
                const shouldPaint = this.skipPaintedPixels ? tileColor === 0 : templateColor !== tileColor;
    ...
    async paint(method = 'linear') {
        ...
        switch (method) {
            ...
            case 'colors-burst-rare':
                log(
                    this.userInfo.id,
                    this.userInfo.name,
                    `[${this.templateName}] 🧱 Painting (Colors burst, rare first)...`
                );
                break;
            case 'random':
            ...
            case 'burst':
                log(
                    this.userInfo.id,
                    this.userInfo.name,
                    `[${this.templateName}] 🧱 Painting (Burst / Multi-source)...`
                );
                break;
        ...
        while (true) {
                ...
                try {
                    ...
                    if (cfg.painting !== false) {
                        log(
                            this.userInfo.id,
                            this.userInfo.name,
                            `[${this.templateName}] 🎲 Mixed mode picked this turn: ${activeMethod}`
                        );
                    }
                } catch (_) {}
            }
                        ...
                        try {
                            ...
                            if (cfg.painting !== false) {
                                log(
                                    this.userInfo.id,
                                    this.userInfo.name,
                                    `[${this.templateName}] 💥 Burst seeds (${desired}): ${JSON.stringify(this._burstSeeds)}`
                                );
                            }
                        } catch (_) {}
                    }
                    ...
                    for (const p of mismatchedPixels) {
                        ...
                        if (!isOutline) {
                            ...
                            for (const [dx, dy] of neigh) {
                                ...
                                if (this.template.data[nx][ny] !== tcol) {
                                    isOutline = true;
                                    break;
                                }
                            }
            ...
            const maxPerPass = Number.isFinite(this.settings?.maxPixelsPerPass)
                ? Math.max(0, Math.floor(this.settings.maxPixelsPerPass))
                : 0;
        ...
        if (res.status === 200 && res.data && res.data.success === true) {
            if (id === 0) {
                log(
                    this.userInfo?.id || 'SYSTEM',
                    this.userInfo?.name || 'wplacer',
                    `[${this.templateName}] 🏳️ Unequipped flag`
                );
            } else {
                log(
                    this.userInfo?.id || 'SYSTEM',
                    this.userInfo?.name || 'wplacer',
                    `[${this.templateName}] 🏳️ Equipped flag #${id}`
                );
            }
            return true;
        }
        ...
        for (let y = 0; y < this.template.height; y++) {
            for (let x = 0; x < this.template.width; x++) {
                ...
                if (shouldPaint) {
                    ...
                    if (templateColor >= 32) {
                        premium++;
                        premiumColors.add(templateColor);
                    } else if (templateColor > 0) {
                        basic++;
                    }
                }
            }
...
function logUserError(error, id, name, context) {
    ...
    if (
        ...
        message.includes('hyper_util::client::legacy::Error') ||
        ...
        log(
            id,
            name,
            `❌ Proxy connection failed - check proxy IP/port or try different proxy (or IP not whitelisted)`
        );
        return;
    }
...
class TemplateManager {
    constructor(
        ...
        this.totalPixels = this.template?.data
            ? this.template.data.flat().filter((p) => (this.paintTransparentPixels ? p >= 0 : p > 0)).length
            : 0;
    ...
    interruptSleep() {
        try {
            if (this._sleepResolver) this._sleepResolver();
        } catch (_) {}
    }
    ...
    async _tryAutoBuyNeededColors() {
        ...
        for (const userId of this.userIds) {
            ...
            try {
                await w.login(u.cookies);
                await w.loadUserInfo();
                const rec = {
                    id: userId,
                    name: w.userInfo.name,
                    droplets: Number(w.userInfo.droplets || 0),
                    bitmap: Number(w.userInfo.extraColorsBitmap || 0),
                };
                candidates.push(rec);
            } catch (e) {
        ...
        const premiumCount = (bitmap) => {
            let c = 0;
            for (let i = 0; i <= 31; i++) if ((bitmap & (1 << i)) !== 0) c++;
            return c;
        };
        ...
        for (const cid of neededColors) {
            ...
            if (ordered.length === 0) {
                ...
                log(
                    'SYSTEM',
                    'wplacer',
                    `[${this.name}] ⏭️ Skip auto-buy color #${cid}: insufficient droplets on all assigned accounts (need ${COLOR_COST} + ${reserve}(reserve) = ${needTotal}).`
                );
            ...
            try {
                ...
                if (before - reserve < COLOR_COST) {
                    /* just in case */ throw new Error('insufficient_droplets');
                }
                ...
                if (this._hasPremium(Number(w.userInfo.extraColorsBitmap || 0), cid)) {
                    log(
                        buyer.id,
                        w.userInfo.name,
                        `[${this.name}] ⏭️ Skip auto-buy color #${cid}: account already owns this color.`
                    );
                    continue;
                }
                log(
                    buyer.id,
                    w.userInfo.name,
                    `[${this.name}] 💰 Attempting to auto-buy premium color #${cid}. Cost 2000, droplets before: ${before}, reserve: ${reserve}.`
                );
                ...
                log(
                    buyer.id,
                    w.userInfo.name,
                    `[${this.name}] 🛒 Auto-bought premium color #${cid}. Droplets ${before} → ${w.userInfo?.droplets}`
                );
                ...
                if (affordableDroplets >= 500) {
                    ...
                    if (target > 0) {
                        log(
                            wplacer.userInfo.id,
                            wplacer.userInfo.name,
                            `💰 Attempting to buy pixel charges (${target}×500) before painting...`
                        );
            ...
            try {
                ...
                if (typeof painted === 'number' && painted > 0) {
                    log(
                        wplacer.userInfo.id,
                        wplacer.userInfo.name,
                        `[${this.name}] ⏰ Estimated time left: ~${this.formatTime(((this.pixelsRemaining - painted) / this.userIds.length) * 30)}`
                    ); //30 seconds for 1 pixel
                }
                ...
                if (error.name === 'SuspensionError') {
                    ...
                    log(
                        uid,
                        uname,
                        `[${this.name}] 🛑 Account suspended until ${suspendedUntilDate}. Removed from the template list.`
                    );
        ...
        try {
            if (!this._initialScanned) {
                ...
                if (useParallel) {
                    ...
                    log(
                        'SYSTEM',
                        'wplacer',
                        `[${this.name}] 🔍 Initial scan (parallel): ${candidates.length} accounts (concurrency=${concurrency}, proxies=${loadedProxies.length}).`
                    );
                    let index = 0;
                    const worker = async () => {
                        for (;;) {
                            ...
                            const w = new WPlacer(
                                ...
                                currentSettings,
                                this.name,
                                this.paintTransparentPixels,
                                this.burstSeeds
                            );
                            try {
                                await w.login(rec.cookies);
                                await w.loadUserInfo();
                                ...
                                log(
                                    w.userInfo.id,
                                    w.userInfo.name,
                                    `[${this.name}] 🔁 Cache update: charges ${cnt}/${mx}`
                                );
                            } catch (e) {
                                logUserError(e, uid, rec?.name || `#${uid}`, 'initial user scan');
                            } finally {
                    ...
                    log(
                        'SYSTEM',
                        'wplacer',
                        `[${this.name}] 🔍 Initial scan: starting (${this.userIds.length} accounts). Cooldown=${cooldown}ms`
                    );
            ...
            while (this.running) {
                ...
                const needFreshSummary =
                    !this._lastSummary || Date.now() - this._lastSummaryAt >= this._summaryMinIntervalMs;
                if (needFreshSummary) {
                    ...
                    try {
                        ...
                        if (this.autoBuyNeededColors) {
                            if (summary.total === 0) {
                                ...
                                try {
                                    autoRes = (await this._tryAutoBuyNeededColors()) || autoRes;
                                } catch (_) {}
                                ...
                                if (anyOwnsRemaining) {
                                    log(
                                        'SYSTEM',
                                        'wplacer',
                                        `[${this.name}] ℹ️ Only premium pixels remain, but some are already owned. Proceeding to paint owned premium while waiting for funds to buy others.`
                                    );
                                } else if (!anyCanBuy) {
                                    const list = Array.from(summary.premiumColors)
                                        .sort((a, b) => a - b)
                                        .join(', ');
                                    ...
                                    log(
                                        'SYSTEM',
                                        'wplacer',
                                        `[${this.name}] ⛔ Template stopped: Only premium pixels remain (${summary.premium} px, colors: ${list}), and none of assigned accounts have enough droplets to purchase (need 2000 + ${reserve2}(reserve) = ${needTotal}).`
                                    );
                ...
                if (this.pixelsRemaining === 0) {
                        ...
                        if (!hasAnyBasic) {
                            ...
                            if (!anyCanBuy) {
                                log(
                                    'SYSTEM',
                                    'wplacer',
                                    `[${this.name}] ⛔ Stopping: Only premium pixels remain and none of assigned accounts have enough droplets to purchase required colors.`
                                );
                            }
                        }
                    }
                    if (this.antiGriefMode) {
                        ...
                        log(
                            'SYSTEM',
                            'wplacer',
                            `[${this.name}] 🖼 Template complete. Monitoring... Next check in ${currentSettings.antiGriefStandby / 60000} min.`
                        );
                ...
                const candidates = this.userIds
                    ...
                    .map((o) => ({
                        uid: o.uid,
                        count: Math.floor(o.pred?.count || 0),
                        max: Math.floor(o.pred?.max || 0),
                    }))
                ...
                for (const { uid: userId } of candidates) {
                    ...
                    if (
                        !resyncScheduled &&
                        ChargeCache.stale(userId, nowSel) &&
                        nowSel - this._lastResyncAt >= this._resyncCooldownMs
                    ) {
                        ...
                        w.login(rec.cookies)
                            .then(() => {
                                try {
                                    log(userId, rec.name, `[${this.name}] ✅ Background resync finished.`);
                                } catch {}
                            })
                            .catch((e) => {
                                logUserError(e, userId, rec.name, 'opportunistic resync');
                                try {
                                    log(
                                        userId,
                                        rec.name,
                                        `[${this.name}] ❌ Background resync finished (error). Try to re-add the account.`
                                    );
                                } catch {}
                            })
                    ...
                    const threshold = currentSettings.alwaysDrawOnCharge
                        ? 1
                        : Math.max(1, Math.floor(p.max * currentSettings.chargeThreshold));
                    ...
                    // Sei - if no users are ready, determine the minimum time we need to wait before checking again.
                        ...
                        if (
                            msWaitUntilNextUser == null ||
                            msWaitUntilNextUser.timeToReady > Math.floor(needBeforeReady - p.count) * 30_000
                        ) {
                            msWaitUntilNextUser = {
                                name: rec.name,
                ...
                if (foundUserForTurn) {
                    ...
                    if (this._lastRunnerId && this._lastRunnerId !== foundUserForTurn) {
                        ...
                        if (passed < ac) {
                            ...
                            log(
                                'SYSTEM',
                                'wplacer',
                                `[${this.name}] ⏱️ Switching account cooldown: waiting ${duration(remain)}.`
                            );
                    ...
                    try {
                        wplacer.shouldStop = () => !this.running;
                    } catch (_) {}
                    try {
                        ...
                        if (pred)
                            log(
                                id,
                                name,
                                `[${this.name}] ▶️ Start turn with predicted ${Math.floor(pred.count)}/${pred.max} charges.`
                            );
                        const paintedNow = await this._performPaintTurn(wplacer);
                        if (typeof paintedNow === 'number' && paintedNow > 0) {
                            try {
                                ChargeCache.consume(foundUserForTurn, paintedNow);
                            } catch {}
                            ...
                            try {
                                log(
                                    id,
                                    name,
                                    `[${this.name}] ℹ️ Nothing painted. Skipping this turn and retrying soon.`
                                );
                            } catch (_) {}
                            await this._sleepInterruptible(5000);
                        }
                        // cache any new seeds
                        this.burstSeeds = wplacer._burstSeeds
                            ? wplacer._burstSeeds.map((s) => ({ gx: s.gx, gy: s.gy }))
                            : this.burstSeeds;
                        saveTemplates();
                        //await this.handleUpgrades(wplacer);
                    } catch (error) {
                        ...
                        if (
                            ...
                            log(
                                foundUserForTurn,
                                userName,
                                `[${this.name}] ❌ Authentication failed (401) - skipping user temporarily`
                            );
                            ...
                            logUserError(
                                error,
                                foundUserForTurn,
                                users[foundUserForTurn]?.name || `#${foundUserForTurn}`,
                                'perform paint turn'
                            );
                        }
                    } finally {
                    ...
                    try {
                        if (this.autoBuyNeededColors) {
                            await this._tryAutoBuyNeededColors();
                        }
                    } catch {}
                    ...
                    log(
                        'SYSTEM',
                        'wplacer',
                        `[${this.name}] ⏳ No users ready. Waiting for next available user (${msWaitUntilNextUser.name}): ${duration(waitTime)}.`
                    );
...
if (!ADMIN_TOKEN) {
    console.warn('⚠️  WARNING: ADMIN_TOKEN environment variable not set. Authentication will be disabled.');
}

const authenticateToken = (req, res, next) => {
    // Strict check for ADMIN_TOKEN - if not configured, deny all API access
    if (!ADMIN_TOKEN || ADMIN_TOKEN.trim() === '') {
        console.error('❌ SECURITY: ADMIN_TOKEN is not configured. All API access denied.');
        return res.status(403).json({ error: 'Server not properly configured - missing admin token' });
    }
    ...
    if (!authHeader) {
        // console.warn(`⚠️ AUTH_FAILED: Missing Authorization header for ${req.method} ${req.path}`);
    ...
    if (parts.length !== 2 || parts[0].toLowerCase() !== 'bearer') {
        // console.warn(`⚠️ AUTH_FAILED: Invalid Authorization format for ${req.method} ${req.path}: ${authHeader}`);
        return res.status(401).json({ error: 'Invalid token format. Use: Bearer YOUR_TOKEN' });
    }

    const token = parts[1];

    if (!token) {
        // console.warn(`⚠️ AUTH_FAILED: Empty token for ${req.method} ${req.path}`);
        return res.status(401).json({ error: 'Token cannot be empty' });
    }

    // Validate token against ADMIN_TOKEN
    if (token !== ADMIN_TOKEN) {
        // console.error(`🚨 AUTH_FAILED: Invalid token provided for ${req.method} ${req.path} from ${req.ip}`);
        return res.status(403).json({ error: 'Invalid or expired token' });
    }

    // Token is valid - proceed to next middleware
    // console.log(`✅ AUTH_SUCCESS: ${req.method} ${req.path} authenticated successfully`);
    next();
};
...
// NOTE: /data directory is now protected by authentication and is NOT exposed as static files

// SSE endpoint for live logs of current session - PROTECTED
app.get('/logs/stream', authenticateToken, (req, res) => {
    try {
...
app.get('/users', authenticateToken, (_, res) => {
...
app.get('/user/status/:id', authenticateToken, async (req, res) => {
    const { id } = req.params;
...
app.post('/users/cleanup-expired', authenticateToken, (req, res) => {
    try {
        ...
        try {
            ...
            try {
                writeFileSync(backupPath, readFileSync(usersPath, 'utf8'));
            } catch (_) {}
            ...
            for (const id of removeIds) {
                if (users[id]) {
                    ...
                    removed++;
                    try {
                        log('SYSTEM', 'Users', `Deleted expired user ${name}#${id}.`);
                    } catch (_) {}
                }
            }
            saveUsers();
...
app.put('/user/:id/update-profile', authenticateToken, async (req, res) => {
    const { id } = req.params;
    if (!users[id] || activeBrowserUsers.has(id)) return res.sendStatus(409);
    ...
    const showLastPixel =
        typeof req.body?.showLastPixel === 'boolean' ? !!req.body.showLastPixel : !!users[id]?.showLastPixel;
    ...
    const willUpdateRemote =
        (name && name !== users[id].name) ||
        discord !== users[id].discord ||
        showLastPixel !== !!users[id].showLastPixel;
...
app.post('/user/:id/open-profile', authenticateToken, (req, res) => {
    try {
        const { id } = req.params;
        const u = users[id];
        ...
        try {
            try {
                log('SYSTEM', 'Profiles', `Launching: ${profilePath}`);
            } catch (_) {}
            ...
            if (braveExe) {
                try {
                    log('SYSTEM', 'Profiles', `Detected Brave: ${braveExe}`);
                } catch (_) {}
            }
...
// --- API: alliance join ---
app.post('/user/:id/alliance/join', authenticateToken, async (req, res) => {
    const { id } = req.params;
    const { uuid } = req.body || {};
    if (!users[id] || activeBrowserUsers.has(id)) return res.sendStatus(409);
    ...
    try {
        ...
        if (status >= 200 && status < 400) {
            ...
            console.error(
                `[Alliance] join failed: user #${id} (${users[id].name}) uuid=${uuid} status=${status} body: ${short}`
            );
        ...
        console.error(
            `[Alliance] join exception: user #${id} (${users[id].name}) uuid=${uuid}:`,
            error?.message || error
        );
        res.status(500).json({ error: error.message });
    } finally {
...
// --- API: alliance leave ---
app.post('/user/:id/alliance/leave', authenticateToken, async (req, res) => {
    const { id } = req.params;
    if (!users[id] || activeBrowserUsers.has(id)) return res.sendStatus(409);
...
app.post('/users/buy-max-upgrades', authenticateToken, async (req, res) => {
    ...
    if (useParallel) {
        ...
        const concurrency = Math.max(1, Math.min(desired, loadedProxies.length || desired, 32));
        console.log(
            `[BuyMax] Parallel mode: ${ids.length} users, concurrency=${concurrency}, proxies=${loadedProxies.length}`
        );
        let index = 0;
        const worker = async () => {
            for (;;) {
                ...
                if (!urec) {
                    report.push({ userId, name: `#${userId}`, skipped: true, reason: 'unknown_user' });
                    buyMaxJob.completed++;
                    continue;
                }
                ...
                try {
                    ...
                    await wplacer.loadUserInfo();
                    buyMaxJob.lastUserId = userId;
                    buyMaxJob.lastUserName = wplacer.userInfo.name;
        ...
        for (let i = 0; i < userIds.length; i++) {
            ...
            if (i < userIds.length - 1 && cooldown > 0) {
                await sleep(cooldown);
            }
        }
    }

    buyMaxJob.active = false;
    buyMaxJob.finishedAt = Date.now();
...
app.post('/users/buy-charges', authenticateToken, async (req, res) => {
    ...
    const doOne = async (userId) => {
        ...
        try {
            await wplacer.login(urec.cookies);
            await wplacer.loadUserInfo();
            buyChargesJob.lastUserId = userId;
            buyChargesJob.lastUserName = wplacer.userInfo.name;
    ...
    try {
        ...
        if (useParallel) {
            ...
            const concurrency = Math.max(1, Math.min(desired, loadedProxies.length || desired, 32));
            console.log(
                `[BuyCharges] Parallel mode: ${ids.length} users, concurrency=${concurrency}, proxies=${loadedProxies.length}`
            );
            let index = 0;
            const worker = async () => {
                for (;;) {
        ...
        buyChargesJob.active = false;
        buyChargesJob.finishedAt = Date.now();
        res.json({ ok: true, cooldownMs: cooldown, reserve: currentSettings.dropletReserve || 0, report });
    } catch (e) {
        buyChargesJob.active = false;
        buyChargesJob.finishedAt = Date.now();
...
app.post('/users/purchase-color', authenticateToken, async (req, res) => {
    try {
        ...
        if (useParallel) {
            ...
            const concurrency = Math.max(1, Math.min(desired, loadedProxies.length || desired, 32));
            console.log(
                `[ColorPurchase] Parallel mode: ${ids.length} users, concurrency=${concurrency}, proxies=${loadedProxies.length}`
            );
            let index = 0;
            const worker = async () => {
                for (;;) {
                    ...
                    try {
                        ...
                        purchaseColorJob.lastUserId = uid;
                        purchaseColorJob.lastUserName = name;
                            ...
                            try {
                                ...
                                if (err?.code === 403 || /FORBIDDEN_OR_INSUFFICIENT/i.test(err?.message)) {
                                    report.push({
                                        ...
                                        name,
                                        skipped: true,
                                        reason: 'forbidden_or_insufficient_droplets',
                                    });
        ...
        purchaseColorJob.active = false;
        purchaseColorJob.finishedAt = Date.now();
...
app.post('/users/flags-check', authenticateToken, async (req, res) => {
    try {
        ...
        if (useParallel) {
            console.log(
                `[FlagsCheck] Parallel: ${ids.length} accounts (concurrency=${concurrency}, proxies=${loadedProxies.length})`
            );
        } else {
        ...
        const doOne = async (uid) => {
            ...
            try {
                ...
                await w.loadUserInfo();
                const u = w.userInfo || {};
                flagsCheckJob.lastUserId = uid;
                flagsCheckJob.lastUserName = u.name;
        ...
        flagsCheckJob.active = false;
        flagsCheckJob.finishedAt = Date.now();
        ...
        res.json({
            ok: true,
            ts: flagsCheckJob.finishedAt || Date.now(),
            cooldownMs: cooldown,
            report: flagsCheckJob.report,
        });
    } catch (e) {
        flagsCheckJob.active = false;
        flagsCheckJob.finishedAt = Date.now();
...
app.get('/users/flags-check/progress', authenticateToken, (req, res) => {
...
app.post('/users/purchase-flag', async (req, res) => {
    try {
        ...
        const doOne = async (uid) => {
            if (!users[uid]) {
                report.push({ userId: uid, error: 'not_found' });
                purchaseFlagJob.completed++;
                return;
            }
            if (activeBrowserUsers.has(uid)) {
                report.push({ userId: uid, error: 'busy' });
                purchaseFlagJob.completed++;
                return;
            }
            activeBrowserUsers.add(uid);
            ...
            try {
                ...
                await w.loadUserInfo();
                const name = w.userInfo?.name;
                purchaseFlagJob.lastUserId = uid;
                purchaseFlagJob.lastUserName = name;
                const beforeDroplets = Number(w.userInfo?.droplets || 0);
        ...
        if (useParallel) {
            ...
            const concurrency = Math.max(1, Math.min(desired, loadedProxies.length || desired, 32));
            console.log(
                `[FlagPurchase] Parallel mode: ${ids.length} users, concurrency=${concurrency}, proxies=${loadedProxies.length}`
            );
            let index = 0;
            const worker = async () => {
                for (;;) {
        ...
        purchaseFlagJob.active = false;
        purchaseFlagJob.finishedAt = Date.now();
...
app.get('/users/purchase-flag/progress', authenticateToken, (req, res) => {
...
app.post('/user/:id/flag/equip', authenticateToken, async (req, res) => {
...
app.post('/users/equip-flag', authenticateToken, async (req, res) => {
    try {
        const { flagId, userIds } = req.body || {};
        ...
        const doOne = async (uid) => {
            ...
            try {
                await w.login(users[uid].cookies);
                await w.loadUserInfo();
                equipFlagJob.lastUserId = uid;
                equipFlagJob.lastUserName = w.userInfo?.name;
                await w.equipFlag(fid);
                await w.loadUserInfo().catch(() => {});
                report.push({
                    userId: uid,
                    ...
                    ok: true,
                    success: true,
                    equippedFlag: Number(w.userInfo?.equippedFlag || fid),
                });
            } catch (e) {
        ...
        if (useParallel) {
            ...
            const concurrency = Math.max(1, Math.min(desired, loadedProxies.length || desired, 32));
            console.log(
                `[FlagEquip] Parallel mode: ${ids.length} users, concurrency=${concurrency}, proxies=${loadedProxies.length}`
            );
            let index = 0;
            const worker = async () => {
                for (;;) {
        ...
        equipFlagJob.active = false;
        equipFlagJob.finishedAt = Date.now();
...
app.get('/users/equip-flag/progress', authenticateToken, (req, res) => {
...
app.post('/users/unequip-flag', authenticateToken, async (req, res) => {
    try {
        ...
        const doOne = async (uid) => {
            ...
            try {
                await w.login(users[uid].cookies);
                await w.loadUserInfo();
                unequipFlagJob.lastUserId = uid;
                unequipFlagJob.lastUserName = w.userInfo?.name;
                await w.equipFlag(0);
                await w.loadUserInfo().catch(() => {});
                report.push({
                    userId: uid,
                    ...
                    ok: true,
                    success: true,
                    equippedFlag: Number(w.userInfo?.equippedFlag || 0),
                });
            } catch (e) {
        ...
        if (useParallel) {
            ...
            const concurrency = Math.max(1, Math.min(desired, loadedProxies.length || desired, 32));
            console.log(
                `[FlagUnequip] Parallel mode: ${ids.length} users, concurrency=${concurrency}, proxies=${loadedProxies.length}`
            );
            let index = 0;
            const worker = async () => {
                for (;;) {
        ...
        unequipFlagJob.active = false;
        unequipFlagJob.finishedAt = Date.now();
...
app.get('/users/unequip-flag/progress', authenticateToken, (req, res) => {
...
app.post('/users/colors-check', authenticateToken, async (req, res) => {
    try {
        ...
        if (useParallel) {
            ...
            const concurrency = Math.max(1, Math.min(desired, loadedProxies.length || desired, 32));
            console.log(
                `[ColorsCheck] Parallel: ${ids.length} accounts (concurrency=${concurrency}, proxies=${loadedProxies.length})`
            );
            let index = 0;
            const worker = async () => {
                for (;;) {
                    ...
                    try {
                        ...
                        colorsCheckJob.report.push({
                            ...
                            level,
                            progress,
                            flagsBitmap: String(u?.flagsBitmap || ''),
                            equippedFlag: Number(u?.equippedFlag || 0),
                        });
                    } catch (e) {
        ...
        console.log(
            `[ColorsCheck] Finished: ${colorsCheckJob.completed}/${colorsCheckJob.total} in ${duration(colorsCheckJob.finishedAt - colorsCheckJob.startedAt)}.`
        );

        res.json({
            ok: true,
            ts: colorsCheckJob.finishedAt || Date.now(),
            cooldownMs: cooldown,
            report: colorsCheckJob.report,
        });
    } catch (e) {
        colorsCheckJob.active = false;
        colorsCheckJob.finishedAt = Date.now();
...
app.get('/users/colors-check/progress', authenticateToken, (req, res) => {
...
app.get('/users/purchase-color/progress', authenticateToken, (req, res) => {
...
app.get('/template/:id', authenticateToken, (req, res) => {
    const { id } = req.params;
    const t = templates[id];
...
app.post('/template', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        templates[templateId].heatmapEnabled = false;
        templates[templateId].heatmapLimit = 10000;
    }
...
app.delete('/template/:id', authenticateToken, async (req, res) => {
    const { id } = req.params;
...
app.put('/template/edit/:id', authenticateToken, async (req, res) => {
    const { id } = req.params;
...
app.delete('/template/:id/heatmap', authenticateToken, (req, res) => {
    const { id } = req.params;
...
app.put('/template/:id', authenticateToken, async (req, res) => {
    const { id } = req.params;
    ...
    if (req.body.running && !manager.running) {
        ...
        try {
            if (typeof manager.interruptSleep === 'function') manager.interruptSleep();
        } catch (_) {}
    }
    res.sendStatus(200);
});
...
app.get('/settings', authenticateToken, (_, res) => res.json({ ...currentSettings, proxyCount: loadedProxies.length }));
app.post('/reload-proxies', authenticateToken, (req, res) => {
    loadProxies();
    res.status(200).json({ success: true, count: loadedProxies.length });
});
app.get('/test-proxies', authenticateToken, async (req, res) => {
    try {
                ...
                try {
                    ...
                    try {
                        log(
                            'SYSTEM',
                            'wplacer',
                            `🧪 Testing proxy #${item.idx} (${item.host}:${item.port}) target=${isMe ? '/me' : '/tile'}`
                        );
                    } catch (_) {}
                    ...
                    try {
                        ...
                        if (String(e && e.name).toLowerCase() === 'aborterror') {
                            ...
                            if (
                                /econnreset|timeout|timed out|socket hang up|enotfound|econnrefused|reqwest::error|hyper_util/i.test(
                                    msg
                                )
                            ) {
                    ...
                    try {
                        ...
                        log(
                            'SYSTEM',
                            'wplacer',
                            `🧪 Proxy #${outcome.idx} ${tag} (${outcome.status}) ${outcome.reason}; ${outcome.elapsedMs} ms`
                        );
                    } catch (_) {}
                }
            }
    ...
    try {
        ...
        const buildProxyUrl = (pp) => {
            ...
            if (pp.username && pp.password)
                s += `${encodeURIComponent(pp.username)}:${encodeURIComponent(pp.password)}@`;
            s += `${pp.host}:${pp.port}`;
            return s;
        };
        ...
        try {
            ...
            try {
                log(
                    'SYSTEM',
                    'wplacer',
                    `🧪 Testing proxy #${idx} (${p.host}:${p.port}) target=${isMe ? '/me' : '/tile'}`
                );
            } catch (_) {}
            const controller = new AbortController();
            const timeoutMs = 10000;
            ...
            try {
                ...
                if (!isMe) {
                    ...
                    if (cloudflareRe.test(text)) {
                        ...
                        outcome.ok = r.status === 200 || r.status === 401;
                        outcome.reason =
                            r.status === 200 ? 'ok_me_200' : r.status === 401 ? 'ok_me_401' : `http_${r.status}`;
                    } else if (r.status === 403) {
                        outcome.ok = false;
                        outcome.reason = 'http_403';
                    } else {
            ...
            try {
                ...
                log(
                    'SYSTEM',
                    'wplacer',
                    `🧪 Proxy #${idx} ${tag} (${outcome.status}) ${outcome.reason}; ${outcome.elapsedMs} ms`
                );
            } catch (_) {}
        }
        res.json(outcome);
...
app.post('/proxies/cleanup', authenticateToken, (req, res) => {
    try {
        const keepIdx = Array.isArray(req.body?.keepIdx)
            ? req.body.keepIdx.map((n) => parseInt(n, 10)).filter((n) => Number.isFinite(n) && n > 0)
            : null;
        const removeIdx = Array.isArray(req.body?.removeIdx)
            ? req.body.removeIdx.map((n) => parseInt(n, 10)).filter((n) => Number.isFinite(n) && n > 0)
            : null;
        ...
        try {
            writeFileSync(backupPath, readFileSync(proxyPath, 'utf8'));
        } catch (_) {}
        ...
        for (const [idx, p] of byIdx.entries()) {
            ...
            const auth =
                p.username && p.password ? `${encodeURIComponent(p.username)}:${encodeURIComponent(p.password)}@` : '';
        ...
        res.json({
            success: true,
            kept: kept.length,
            removed: byIdx.size - kept.length,
            backup: path.basename(backupPath),
            count: loadedProxies.length,
        });
    } catch (e) {
        res.status(500).json({ error: String((e && e.message) || e) });
    }
...
app.get('/canvas', authenticateToken, async (req, res) => {
...
// --- API: version check ---
app.get('/version', authenticateToken, async (_req, res) => {
    try {
...
app.get('/changelog', authenticateToken, async (_req, res) => {
    try {
        let local = '';
        try {
...
const keepAlive = async () => {
    ...
    if (useParallel) {
        ...
        log(
            'SYSTEM',
            'wplacer',
            `⚙️ Performing parallel keep-alive for ${candidates.length} users (concurrency=${concurrency}, proxies=${loadedProxies.length}).`
        );
...
app.get('/export-tokens', authenticateToken, (req, res) => {
    try {
...
(async () => {
    ...
    for (const id in loadedTemplates) {
        ...
        if (t.userIds?.every((uid) => users[uid])) {
            ...
            try {
                ...
                tm.heatmapEnabled = false;
                tm.heatmapLimit = 10000;
            }
    ...
    if (host === '0.0.0.0') {
        const securityWarning1 =
            '⚠️  SECURITY WARNING: HOST=0.0.0.0 makes the server accessible from outside your computer!';
        const securityWarning2 =
            '   For better security, change HOST to 127.0.0.1 in .env file or use run-localhost.bat';
